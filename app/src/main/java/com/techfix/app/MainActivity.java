...
public class MainActivity extends AppCompatActivity {
    ...
    void build(){
        ...
        root.addView(nav);
        setContentView(root);
        ViewCompat.setOnApplyWindowInsetsListener(root,(v,insets)->{int statusTop=insets.getInsets(WindowInsetsCompat.Type.statusBars()).top;int navBottom=insets.getInsets(WindowInsetsCompat.Type.navigationBars()).bottom;statusTop=Math.max(statusTop,dp(8));root.setPadding(0,statusTop,0,0);nav.setPadding(dp(6),dp(7),dp(6),Math.max(dp(7),navBottom));return insets;});
